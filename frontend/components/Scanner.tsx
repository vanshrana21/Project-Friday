...
export default function Scanner({ onScanComplete }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700 transition-all hover:shadow-xl">
      ...
      <div 
        id="qr-reader" 
        className={`w-full min-h-[250px] bg-gray-100 dark:bg-gray-900 rounded-xl overflow-hidden border-2 border-gray-300 dark:border-gray-700 mb-4 flex items-center justify-center ${isScanning ? 'shadow-[0_0_20px_rgba(59,130,246,0.6)]' : ''}`}
      >
        {!isScanning && (
      ...
      {showManual && (
        <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700">
      ...
      <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium mt-4 ${
        ...
        <span className={`w-2 h-2 rounded-full ${isScanning ? 'bg-yellow-500 animate-pulse' : 'bg-gray-400'}`} />
        {scanStatus}
      </div>
    </div>
  );
